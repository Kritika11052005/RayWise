...
const unsaveInstallerRecommendation = useMutation(api.savedRecommendations.unsaveInstallerRecommendation);
  ...
  const fetchRecommendations = async () => {
    ...
    toast.warning('Maximum budget must be greater than minimum budget');
      return;
    }
  ...
  const handleSavePanelRecommendation = async (panel: PanelRecommendation) => {
    ...
    try {
      ...
      toast.success(`✓ ${panel.name} saved to your recommendations!`);
    } catch (error) {
      console.error('Error saving panel:', error);
      toast.error(error instanceof Error ? error.message : 'Failed to save panel recommendation');
    } finally {
  ...
  const handleSaveInstallerRecommendation = async (installer: InstallerRecommendation) => {
    ...
    try {
      ...
      toast.success(`✓ ${installer.company} saved to your recommendations!`);
    } catch (error) {
      console.error('Error saving installer:', error);
      toast.error(error instanceof Error ? error.message : 'Failed to save installer recommendation');
    } finally {
      setSavingRecommendation(null);
    }
  };
...
const handleUnsavePanelRecommendation = async (panel: PanelRecommendation) => {
    ...
    try {
      ...
      toast.success(`✓ ${panel.name} removed from your saved recommendations`);
    } catch (error) {
      console.error('Error unsaving panel:', error);
      toast.error(error instanceof Error ? error.message : 'Failed to unsave panel recommendation');
    } finally {
      setSavingRecommendation(null);
    }
  };
...
const handleUnsaveInstallerRecommendation = async (installer: InstallerRecommendation) => {
    ...
    try {
      ...
      toast.success(`✓ ${installer.company} removed from your saved recommendations`);
    } catch (error) {
      console.error('Error unsaving installer:', error);
      toast.error(error instanceof Error ? error.message : 'Failed to unsave installer recommendation');
    } finally {
      setSavingRecommendation(null);
    }
  };
  const handleSaveSolution = async () => {
    if (!finalizedLayoutId) {
      toast.warning('Please finalize your layout first');
      return;
    }

    if (selectedPanelIndex === null && selectedInstallerIndex === null) {
      toast.warning('Please select at least a panel type or an installer');
      return;
    }
    ...
    try {
      ...
      toast.success('Solution saved successfully! Your selected installer will be notified.');
    } catch (error) {
      console.error('Error saving solution:', error);
      toast.error('Failed to save solution: ' + (error instanceof Error ? error.message : 'Unknown error'));
    } finally {
      setSaving(false);
    }
  };
