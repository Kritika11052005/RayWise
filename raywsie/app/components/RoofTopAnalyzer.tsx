...
const currentUser = useQuery(api.users.getCurrentUser);
  ...
  const [showDeleteConfirm, setShowDeleteConfirm] = useState<'project' | 'finalized' | null>(null);
  const [modalInputs, setModalInputs] = useState({ name: '', description: '' });
  // Convex mutations
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          (error) => {
            ...
            toast.error('Unable to detect location', {
              description: 'Please check location permissions or enter manually'
            });
            setDetectingLocation(false);
          }
        );
  ...
  const fetchSatelliteImage = async (lat?: number, lon?: number) => {
    ...
    if (!latitude || !longitude) {
      if (locationInput.trim()) {
        ...
        toast.warning('Please enter a location first');
      }
      return;
    }
  ...
  const fetchSatelliteImageWithCoords = async (lat: number, lon: number) => {
    ...
    try {
      ...
      toast.error(`Failed to fetch satellite image: ${error instanceof Error ? error.message : 'Unknown error'}\n\nPlease try:\n- A different location\n- Uploading your own image instead`);
      setFetchingMapImage(false);
    }
  };
    ...
    if (locationInput.trim()) {
      ...
      if (imageSource === 'map') {
        ...
        if (coords) {
          ...
          toast.error('Could not find this location. Please try:\n\n' +
...
const analyzeRooftop = async () => {
  if (!image || polygonPoints.length < 3) {
    toast.warning('Please upload an image and draw a polygon area first');
    return;
  }
  ...
  const handleSaveProject = async () => {
    if (!location || !imageUrl || polygonPoints.length < 3) {
      toast.warning('Missing requirements', {
        description: 'Please set location, upload image, and draw polygon area'
      });
      return;
    }
...
const handleFinalizeLayout = async () => {
    if (!location || !analysis || !panelLayout.length || polygonPoints.length < 3) {
      toast.warning('Incomplete analysis', {
        description: 'Please complete: Set location, Draw polygon, Run AI analysis'
      });
      return;
    }
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        toast('Enter layout name', {
          action: {
            label: 'Finalize',
            onClick: () => {
              const input = document.querySelector('input[data-layout-name]') as HTMLInputElement;
              layoutName = input?.value || '';
              resolve();
            }
          },
          cancel: {
          ...
          duration: Infinity,
          closeButton: false,
          description: <input data-layout-name type="text" placeholder="Layout name" className="w-full mt-2 px-3 py-2 border rounded" />
        });
      });
      ...
      await new Promise<void>((resolve) => {
        toast('Enter description (optional)', {
          action: {
            label: 'Continue',
            onClick: () => {
              const input = document.querySelector('input[data-layout-desc]') as HTMLInputElement;
          ...
          duration: Infinity,
          closeButton: false,
          description: <input data-layout-desc type="text" placeholder="Description" className="w-full mt-2 px-3 py-2 border rounded" />
        });
      });
      ...
      toast.success('Layout finalized successfully!');
    } catch (error) {
  ...
  const handleDeleteProject = async () => {
    if (!currentProjectId) {
      toast.warning('No saved project to delete');
      return;
    }

    await new Promise<void>((resolve) => {
      toast.warning('Confirm deletion', {
        description: 'Are you sure you want to delete this saved project? This cannot be undone.',
        action: {
          label: 'Delete',
          onClick: async () => {
            try {
  ...
  const handleDeleteFinalizedLayout = async () => {
    if (!currentFinalizedId) {
      toast.warning('No finalized layout to delete');
      return;
    }

    await new Promise<void>((resolve) => {
      toast.warning('Confirm deletion', {
        description: 'Are you sure you want to delete this finalized layout? This cannot be undone.',
        action: {
          label: 'Delete',
          onClick: async () => {
            try {
              ...
              setCurrentFinalizedId(null);
              toast.success('Finalized layout deleted successfully!');
            } catch (error) {
              console.error('Delete finalized layout error:', error);
              toast.error('Failed to delete finalized layout', {
                description: error instanceof Error ? error.message : 'Unknown error'
              });
            }
            resolve();
