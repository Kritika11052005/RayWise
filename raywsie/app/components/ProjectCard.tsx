...
export default function ProjectCard({ project, onDelete }: ProjectCardProps) {
  ...
  const handleConvertToFinalized = async () => {
    if (project.status !== "analyzed" || !project.analysis || !project.panelLayout) {
      toast.warning("Project must be analyzed before finalizing!");
      return;
    }
    ...
    try {
      ...
      toast.success(`Successfully converted "${project.name}" to finalized layout!`);
      ...
      toast.error("Failed to convert project. Please try again.");
    } finally {
      setConverting(false);
    }
  };
